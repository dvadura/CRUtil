 ...
 *
 * @details String literals are XOR-encoded at compile time using LString::encode(),
 ...
 *          LString::decode() to recover the original string.
 *
 *          Usage:
 *
 ...
 *                { 1, LString::encode("this is a test") }
 *             };
 *
 *          Based on:
 *
...
namespace crutil {
   // Forward declaration (needed by LString::encode)
   ...
   struct LString {
      ...
      static std::string decode(const lstring<N>& enc) {
         constexpr LString o;
         std::string result;
         result.resize(N - 1);
         for (std::size_t i = 0; i < N - 1; ++i) {
            result[i] = o(enc.data[i], i);
         }
         return result;
      }
   ...
   template <int M, typename T=char> struct sdata {
      ...
      template <typename TT=T, std::size_t... I>
         constexpr
            sdata(const TT (&v)[M], ilist<I...>) : data{ LString()(v[I], I)... } {}
   };
   ...
   template <std::size_t N>
      struct lstring {
         ...
         template <std::size_t... I>
            constexpr
               lstring(const char (&v)[N], ilist<I...>) : data{ LString()(v[I], I)... } {}
      };

   // Out-of-line definition of LString::encode (requires lstring to be complete)
   template <int N>
      constexpr
         lstring<N> LString::encode(const char (&s)[N]) {
